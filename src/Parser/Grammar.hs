...
import qualified Lexer (TokenType (..))
...
data Nonterminal = Nonterminal String | Augment deriving (Show, Eq, Ord, TH.Syntax.Lift)

type Terminal = Lexer.TokenType
...
nt :: String -> TH.Q TH.Type -> GrammarMonad Nonterminal
nt t ty = do
    let nt = Nonterminal t
    GrammarMonad $ State.state $ \gms -> ((), gms {gms_nt_result_tys = Map.insert nt ty (gms_nt_result_tys gms)})
    pure $ Nonterminal t

get_nt_ty :: Nonterminal -> GrammarMonad (TH.Q TH.Type)
get_nt_ty nt = do
    gms <- GrammarMonad State.get
