...
import qualified Parser.Error as Error (Error (..))
import qualified Safe
import Control.Monad.Writer (lift, runWriterT, tell)

import qualified Lexer (Token (..), TokenType (..), to_token_type)
...
data ItemSet = ItemSet Int (Set Item) (Set Item) deriving (Show)
data Item = Item Rule Int Terminal deriving (Eq, Ord, Show)

-- instance Format Item where
--     format (Item (Rule _ nt pr) i l) =
--         let (before, after) = splitAt i pr
--         in format nt <> " -> " <> String.intercalate " " (map format before <> ["."] <> map format after) <> " {" <> format l <> "}"
-- instance Format ItemSet where
--     format (ItemSet n k c) =
--         "item set "
--             <> show n
--             <> " { "
--             <> String.intercalate ", " (map format (Set.toList k))
--             <> " ; "
--             <> String.intercalate "," (map format (Set.toList c))
--             <> " }"
...
data ActionOrConflict t = SingleAction t | Conflict [t] deriving (Functor, Show)
...
--     format (Conflict as) = String.intercalate "/" (map format as)
...
find_sets grammar item_set_tables (current_set_number : to_process) = do
    ...
    where
        intern kernel closure = StateMonad.state $ \sets ->
...
convert_to_state_table nt_result_types reduce_fn_map item_sets =
    ...
    where
                                        ...
                                        error $
                                            "conflict in parsing table: "
                                                ...
                                                <> intercalate
                                                    "\n"
                                                    ( map
                                                        (\(n, (item_set, actions, gotos)) -> show n <> " | " <> show item_set <> " | " <> format_map actions <> " " <> format_map gotos)
                                                        (Map.toList states)
                                                    )
                                )
                        )
        ...
        format_map m = "{ " <> intercalate ", " (map (\(k, v) -> show k <> ": " <> show v) (Map.toList m)) <> " }"
...
generate_table grammar =
    convert_to_state_table (nt_result_types grammar) (reduce_fn_map grammar) $
        StateMonad.evalState
            ...
            [new_item_set grammar 0 (Set.singleton $ new_item (augment_rule grammar) Lexer.TT'EOF)]
...
parse_loop ::
    Typeable.Typeable a =>
    (Lexer.Token -> Int -> Lexer.TokenType -> Either Error.Error ParserAction) ->
    [Lexer.Token] ->
    Either Error.Error a
parse_loop action_table = helper [0] []
    where
        helper state_stack@(current_state : _) ast_stack tokens =
            let (current_token, more_tokens) =
                    case tokens of
                        (current_token:more_tokens) -> (current_token, more_tokens)
                        [] -> (Lexer.EOF, [])
            in case action_table current_token current_state (Lexer.to_token_type current_token) of
                ...
                Right (PA'Reduce reduce) ->
                    ...
                    in helper next_state_stack next_ast_stack (current_token : more_tokens)
...
make_parse_fn name res_ty (StateTable nt_ty_map reduce_fn_map table) = do
                                            ...
                                            ( TH.normalB
                                                [|
                                                    let (last_state, state_stack_popped, asts_popped, ast_stack_popped) = pop_from_stack $(TH.litE $ TH.IntegerL $ toInteger prod_len) $(TH.varE state_stack_name) $(TH.varE ast_stack_name)
                                                        next_state = $(TH.varE goto_table_name) last_state $(TH.Syntax.lift $ stringify_nt nt)
                                                        new_ast =
                                                            $( foldlM
                                                                ( \e (sym_i, _) ->
    ...
    top_sig <- TH.SigD fn_name <$> [t|[Lexer.Token] -> Either Error.Error $res_ty|]
    ...
    where
            ...
            decl <-
                TH.funD
                    ...
                    ( map_flatten_map
                        ( \state nt goto ->
                            TH.clause
                                [TH.litP $ TH.IntegerL $ toInteger state, TH.litP $ TH.StringL $ stringify_nt nt]
        ...
        action_table_function reduce_fn_names name = do
            ...
            sig <- TH.sigD name [t|Lexer.Token -> Int -> Lexer.TokenType -> Either Error.Error ParserAction|]
            decl <-
                TH.funD
                    name
                            ...
                            ( \(state, actions) -> do
                                ...
                                let default_clause = do
                                        ...
                                        TH.clause
                                            ...
                                            (TH.normalB [|Left (Error.BadToken $(TH.litE $ TH.IntegerL $ toInteger state) $(TH.varE tok_name))|])
                                            []
                                actions
                                    & Map.toAscList
                                    & map
                                        ( \(terminal, action) -> do
                                            let terminal_ctor_name_str = "Lexer.TT'" <> term_name terminal
                                            terminal_ctor_name <-
        ...
        sym_ty (S'T _) = [t|Lexer.Token|]
        sym_ty (S'NT nt) = nt_ty_map Map.! nt
        stringify_nt :: Nonterminal -> String
